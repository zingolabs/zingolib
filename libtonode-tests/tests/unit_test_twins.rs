...
TransactionSummary {
    ...
    // NOTE: from_t_z_o_tz_to_zo_tzo_to_orchard is temporarily restored
    // to concrete.rs's default suite: its offline twin is ignored on
    // zingolabs/zingolib#2447, and the twin-and-verify contract requires
    // one copy of the protection running by default. When the twin
    // revives, the live original returns here behind this gate.
