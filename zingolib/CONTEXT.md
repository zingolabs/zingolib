...
**Correspondent Pools** — The supply structures of Exit Rotation, one per Correspondable kind: the **Indexer Pool** and the **Price Source Pool**. Each holds Correspondent-Bound transports ready for its kind's Transmissions — two in the Indexer Pool (Exit Rotation's ratified complement; the Price Source Pool's size is not yet ratified). A Transmission consumes one member whole; consumption or death triggers a background PrioritisePrivacy refill. Admission requires a bound exit and an assigned Correspondent, never a probe. Replaces the working term Standing Pool 2026-08-08. _Avoid_: Standing Pool.
