...
**Health** — The wallet's per-indexer judgment of whether an endpoint will answer, derived from evidence — the recorded contact attempts — and that evidence's age. The Server-Selection Sweep sets the boot baseline; thereafter every Transmission doubles as a probe and updates the Health of the indexer it contacted. Session-scoped, like all failure evidence. _Avoid_: liveness for this judgment (kept only for process-liveness senses such as the Transmit Heartbeat); warmth in any sense. Ratified 2026-08-08.
