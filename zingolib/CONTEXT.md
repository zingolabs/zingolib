...
**Correspondent Pools** — The supply structures of Exit Rotation, one per Correspondable kind: the **Indexer Pool** and the **Price Source Pool**. Each holds Correspondent-Bound transports ready for its kind's Transmissions — two in the Indexer Pool (Exit Rotation's ratified complement), one in the Price Source Pool, whose refill always finishes inside the fetch cadence. A Transmission consumes one member whole; consumption or death triggers a background PrioritisePrivacy refill. Admission requires a bound exit and an assigned Correspondent, never a probe. Replaces the working term Standing Pool 2026-08-08. _Avoid_: Standing Pool.

**Price Fetch Cadence** — The session-scoped rhythm of an opted-in price fetch: off, or on for the session, firing at an interval drawn uniformly from four to five minutes — a four-minute floor plus a uniform draw across the fifth minute — so no observer can anchor the wallet's clock to a fixed period. Every run selects a new Exit Node: the refill's draw excludes the previous run's exit, so consecutive fetches never share one even by re-draw chance, a hard rule where Exit Rotation's recycling alone is only probabilistic. Each firing is an ordinary Transmission consuming the Price Source Pool's member, and so also the session's steady source of price-side Health evidence. A deliberate, ratified wallet-side clock. Ruled 2026-08-08.
