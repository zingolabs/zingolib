...
**Correspondent Pools** — The supply structures of Exit Rotation, one per Correspondable kind: the **Indexer Pool** and the **Price Source Pool**. The Indexer Pool holds two Exit-Bound transports (Exit Rotation's ratified complement), each consumed whole by one pull of a Transmission, which binds its Correspondent as it dials. The Price Source Pool holds one Shared-exit transport (ruled 2026-08-08): a price run fans out over it to the whole price census, and the member is consumed by the run, never bound to one source. Consumption or death triggers a background PrioritisePrivacy refill, which always finishes inside the fetch cadence. Admission requires a bound exit — and, for the Indexer Pool only, an assigned Correspondent — never a probe. Replaces the working term Standing Pool 2026-08-08. _Avoid_: Standing Pool.

**Price Fetch Cadence** — The session-scoped rhythm of an opted-in price fetch: off, or on for the session, firing at an interval drawn uniformly from four to five minutes — a four-minute floor plus a uniform draw across the fifth minute — so no observer can anchor the wallet's clock to a fixed period. Every run selects a new Exit Node: the refill's draw excludes the previous run's exit, so consecutive runs never share one even by re-draw chance, a hard rule where Exit Rotation's recycling alone is only probabilistic. Within a run the fetch is a saturating race — one Exit Node, every price source pulled at once, the first quote winning and the losers cancelled — chosen for response speed; the exit's many-destination view is ADR 0039's accepted Shared exposure, since the traffic names no wallet (ruled 2026-08-08). The cadence itself is the consumer's contract, not a wallet-layer clock: the library enforces the per-run invariants whenever a consumer fires a fetch, and zingo-cli and mobile implement the rhythm. Each run is also the session's steady source of price-side Health evidence. Ruled 2026-08-08.

**Exit-Bound** — The first rung of a transport's life: its Exit Node is bound and its local tunnel is up, and no destination has yet been observed through it. The exit identity is announced to the wallet at bind time, so concurrent draws can exclude it from the moment it is held. Ratified 2026-08-08.

**Correspondent-Bound** — The second rung: an Exit-Bound transport that a pull has bound to its one Correspondent by dialing it. Binding happens at consumption, never at admission, because a pool of two cannot pre-bind an escalation of six, and a Correspondent drawn minutes early is drawn against stale Health (ruled 2026-08-09). The binding lasts the transport's remaining life, which is that one Transmission: its exit observes one destination ever, and its Correspondent sees one contact ever. Ratified 2026-08-08; binding moved to consumption 2026-08-09.

**Exit Rotation** — The send-privacy property whereby consecutive Transmissions never share a mixnet exit, so the exit never becomes a linking key at the destination. The wallet keeps Correspondent Pools of Exit-Bound transports; each pull of a Transmission consumes one, and a replacement is brought up in the background, so rotation never costs a send the tunnel-establishment wait. A pool member is never probed: nothing is dialed through it until the pull that consumes it, and so the exit observes exactly one destination in its life and the Correspondent sees exactly one contact (ruled 2026-08-08). An empty pool never lends a send another surface's tunnel: the send waits on its own PrioritisePrivacy acquisition (ruled 2026-08-08). Complements Correspondent Selection on the other side of the tunnel: the Correspondent differs per send, and so does the exit that delivers to it. Ratified 2026-07-28; mechanism not yet implemented.
...
**Maintained Indexer Pool** — SUPERSEDED 2026-08-08 by the Correspondent Pools and the transmissions-are-probes Health economy: transports are consumed per Transmission and replenished, never maintained to every endpoint, and an indexer's standing derives from Health evidence rather than from a held connection. Historically (ratified 2026-08-04) it named a standing set of mixnet connections to every eligible census endpoint, carrying the send escalation, Correspondent duties, and price. What survives of it: the retirement of server selection as a concept, which lives on in the Server-Selection Sweep. _Avoid_: Maintained Indexer Pool for the Correspondent Pools. See `docs/adr/0029-a-maintained-mixnet-indexer-pool-replaces-server-selection.md`.
...
**Session Retirement** — SUPERSEDED 2026-08-10: the mechanism is excised (ADR 0038 amendment). Historically (ratified 2026-08-07) it named the session-scoped, evidence-based withdrawal of an Exit Node whose failure rate stood more than one standard deviation above the pool's mean. The statistic retired a node on its first charged failure over a mostly-zero population (issue #2661, finding 5), so the wallet now assumes every discovered Exit Node is somewhat viable, and exclusion follows use alone. Any future exit-quality filter adopts the upstream directory's performance annotations, never an in-wallet statistic. _Avoid_: Session Retirement for any live mechanism.
