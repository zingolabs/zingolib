...
**Mixnet Sync Window** — The most recent segment of the chain (7,000 blocks, one week at the 75-second target spacing) whose compact-block sync routes over the Nym mixnet, fail-closed under Mixnet Mode exactly like Transmission. Below the window, sync is the user's transport territory: it rides the session's existing route, which is the user's system-level NymVPN when one runs and clearnet otherwise; the wallet never embeds a dVPN. The window is sized so a worst-case week-long catch-up fits a mobile background-sync slot, which in some cases runs about five minutes a night, while still covering the range where timing linkage lives. Ratified 2026-07-28. See `docs/adr/0023-top-window-sync-rides-the-mixnet.md`.

**Sync Session** — One sync run, from start to quiescence. The unit of sync freshness: each Sync Session gets a dedicated mixnet tunnel with its own fresh exit (never drawn from Exit Rotation's send pool) and a freshly selected sync indexer. A broadcast can therefore never share a transport, an exit, or a session with sync. Ratified 2026-07-28.
