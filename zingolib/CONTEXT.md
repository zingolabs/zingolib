...
**Correspondent-Bound** — The second rung: an Exit-Bound transport assigned its one Correspondent by draw, still having dialed nothing. The binding is an assignment, not a contact, and it lasts the transport's life: the transport's first and only dial is the Transmission that consumes it, so its exit observes one destination ever and its Correspondent sees one contact ever. Ratified 2026-08-08.

**Exit Rotation** — The send-privacy property whereby consecutive Transmissions never share a mixnet exit, so the exit never becomes a linking key at the destination. The wallet keeps Correspondent Pools of Correspondent-Bound transports; a Transmission consumes one, and a replacement is brought up in the background, so rotation never costs a send the tunnel-establishment wait. A pool member is never probed: its Correspondent is assigned by draw at admission, nothing is dialed until the Transmission itself, and so the exit observes exactly one destination in its life and the Correspondent sees exactly one contact (ruled 2026-08-08). An empty pool never lends a send another surface's tunnel: the send waits on its own PrioritisePrivacy acquisition (ruled 2026-08-08). Complements Correspondent Selection on the other side of the tunnel: the Correspondent differs per send, and so does the exit that delivers to it. Ratified 2026-07-28; mechanism not yet implemented.
