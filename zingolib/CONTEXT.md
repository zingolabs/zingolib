...
**MixnetConduit** — What the wallet holds when it has somewhere to send mixnet traffic, asked for by role and opaque to the wallet: it can name one and pass one back to have work carried, and it can read nothing out of one. The provider decides how many mixnet clients a set of roles takes, so a provider answering every role with the same conduit serves them all over one client while a provider answering with distinct conduits serves each over its own, and the wallet cannot tell those apart. A conduit is anchored to one proven exit for the epoch rather than drawing a fresh path per request, which is why a role's proof survives the client that earned it — but the anchoring is the provider's business and the wallet never learns which exit, or that there is an exit at all. *Avoid*: SlotTunnel and the term "tunnel" generally (the retired name, retired 2026-08-18); ExitAnchored (rejected: it names the wallet's one exit-free type after an exit). Ratified 2026-08-18; see `docs/adr/0046-the-wallet-asks-for-a-mixnet-conduit-by-role.md`.
