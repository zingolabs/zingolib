...
**Unqualified** — Outside a Qualified Range, in one of two distinct senses. An **unqualified reading binary** is a running wallet binary whose Qualified Range does not cover the wallet or chain before it: it refuses at wallet read, touching nothing. **Unqualified written** data is a wallet entry recorded at a height outside *its writer's* Qualified Range: a qualified reading binary quarantines it and heals by truncation to the end of the poisoning writer's greatest understood epoch. *Avoid*: unqualified scanner (the refusal happens at read, before any scanning exists), invalid, stale (both blur which party is at fault).
