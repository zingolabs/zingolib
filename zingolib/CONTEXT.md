...
**Narration** — Human-facing progress reporting emitted while a long-running operation is in flight: the transmit path's latest progress line, a migration batch's build/send counts, server probing. Narration is presentation, never data: it carries no parse contract, its wording may change in any release, and in zingo-cli it travels only on stderr, because stdout carries exactly the command's result (see `docs/adr/zingolib/0031`). The library publishes narration pull-style through progress handles; each frontend decides its own cadence and rendering. *Avoid*: parsing narration lines (typed frontends poll the progress handles; machine consumers read the stdout result).
...
**Destination Selection** — The draw that decides which Destination receives a Transmission: random, under three constraints, run by the escalation as each pull launches, so every draw judges current Health (ruled 2026-08-09). The sync indexer's operator is never eligible (see Destination Indexer), and the Destination must differ from the previous Transmission's, remembered for the session only and never persisted. Selection governs the Indexer kind alone: a price run contacts its whole census over a Shared exit and draws nothing (ruled 2026-08-08). The property this buys is non-accumulation: no single indexer collects a record of the user's consecutive sends. Selection consumes nothing: a draw neither consumes nor replaces a transport. Ratified 2026-07-28; renamed from Witness Selection 2026-08-07. _Avoid_: Witness Selection.

**Destination** — The served party a Transmission is submitted to: an Indexer that receives a transaction and relays or suppresses it, a Price Source that answers a quote — whoever answers the one targeted request and thereby learns it was made. Replaces "witness" in the send-privacy vocabulary (ratified 2026-08-07), because a witness is by definition not a party, while the Destination is a principal — served, acting, and held to account by the delivery check. The role was first minted from banking's correspondent — the institution that acts on another's behalf at a remove — and renamed to the plain Destination 2026-08-25 (ADR 0036 amendment). Of the anonymity literature's rejected candidates, "counterparty" collides with a payment's payee, and "observer" names a watching adversary rather than a served party. Bare "witness" remains the Merkle authentication path inside sync code, and only there. _Avoid_: witness (for this role), Correspondent, broadcast witness, observer.

**Destination Rotation** — The send-privacy property governing a single Transmission's escalation: on the happy path exactly one Destination receives the transaction, and the Transmission widens only on evidence, up to a fixed cap of distinct Destinations. The escalation is a hedged race of full paths in the RFC 8305 style: each arm pairs a freshly drawn Destination (by Destination Selection) with its own pull over the session's standing client, a further arm launches only after a silence interval or a failure, the interval is chosen long enough that a responsive Destination's confirmed delivery wins before a second arm ever launches, and the first confirmed delivery wins with every other arm cancelled. This guards against a censoring Destination that accepts a submission but suppresses, stalls, or misreports the relay — a silent stall now costs one hedge interval, not a full timeout. Renamed from Witness Rotation 2026-08-07. Ruled 2026-08-08: the hedged race replaces the serially gated one-two-three rounds of `docs/adr/zingolib/0011-nym-mixnet-transmission.md`; the cap and the adversary model stand. _Avoid_: Witness Rotation, Witness Selection (now Destination Selection); "rounds" for this escalation. See `docs/adr/zingolib/0040-sends-escalation-is-a-hedged-race-of-full-paths.md`.

**Health** — The wallet's per-indexer judgment of whether an endpoint will answer, derived from evidence — the recorded contact attempts — and that evidence's age. The Server-Selection Sweep sets the boot baseline; thereafter every Transmission doubles as a probe and updates the Health of the indexer it contacted. A failure counts only where the evidence names the party: a tunnel-phase failure is the Exit Node's and never charges the Destination, and a failure the phase cannot attribute charges nobody. Health governs a Destination draw as a binary eligibility filter under a floor, never as a weight: weighting would make a uniform draw non-uniform and so more fingerprintable, and the floor stops a partition from shrinking the anonymity set below a minimum eligible pool. Exit Nodes are judged instead by the NodeHealthIndex's Observations, whose harder withholding is safe because that population is large and fungible. Session-scoped, like all failure evidence, and kept only in memory; the session's indexer history holds the same attempts beside it and is equally session-scoped, so neither the judgment nor the record of which indexers this wallet contacted survives the process (ruled 2026-08-17, retiring both the at-rest export and the term _Indexer Diary_ that named it). _Avoid_: liveness for this judgment (kept only for process-liveness senses such as the Transmit Heartbeat); warmth in any sense. Ratified 2026-08-08; the charge rule, the filter-under-floor, and the memory-first posture ruled 2026-08-09.

**Correspondable** — Something that can be corresponded with over the mixnet: the party a Transmission addresses, never the path that carries it. The Indexer and the Price Source are the Correspondable kinds; a mixnet node — mix, gateway, or Exit Node — is infrastructure and is never Correspondable. Every Correspondable is addressed by an https URI and carries an accountable operator where one is known. Ratified 2026-08-08; the name deliberately survives the 2026-08-25 Correspondent→Destination sweep (ruled 2026-08-25).
...
**Indexer Operator** — The party in administrative control of an indexer endpoint, derived as the endpoint host's registrable domain: one domain is one administrative authority, so endpoints sharing a domain share an operator. The derivation is sound in that direction only — distinct domains are merely the absence of evidence of common control, never proof of it — and policies that need operator disjointness lean on that weaker direction; where a pairing is privacy-critical, distinctness deserves recorded attestation, and infrastructure correlations (a shared custom build, a shared certificate chain) count against it. Ratified 2026-08-04. See `docs/adr/zingolib/0029-a-maintained-mixnet-indexer-pool-replaces-server-selection.md`.
...
**Exit Role** — The job one Exit Node holds for a Nym epoch. Boot proves four exits and assigns each a role in the order it confirms, so the role belongs to the exit rather than to any client: a client is the transient instrument that binds an exit and does one job, while the exit keeps its role and its `EpochProven` proof until the epoch ends or it starts failing. Health decides whether an exit may be used at all; the Exit Role decides which of the proven four a given operation reaches for. The four roles are the IndexerSweep exit, the PriceFetch exit, the IndexerClient exit, and the spare exit. *Avoid*: naming a role after the client that binds it — only the client on the IndexerClient exit outlives its task. This holds below the mixnet seam and becomes invisible above it, where the wallet asks for a MixnetConduit by role and the role belongs to the job because no exit is nameable there. Ratified 2026-08-17; amended 2026-08-18 by `docs/adr/zingolib/0046-the-wallet-asks-for-a-mixnet-conduit-by-role.md`; see `docs/adr/zingolib/0045-boot-proves-four-exits-and-assigns-them-by-role.md`.

**MixnetConduit** — What the wallet holds when it has somewhere to send mixnet traffic, asked for by role and opaque to the wallet: it can name one and pass one back to have work carried, and it can read nothing out of one. One thing escapes, and for the human alone: a conduit carries a label, which the session's status reports in place of a typed exit identity. The label is `Display` and nothing else. It carries no equality, no ordering, no hashing, no borrow to its text, and no deserialization back into an identity, so a caller can print it and can do nothing else with it. A caller that needs to compare two labels has thereby shown that the responsibility belongs below the seam. The provider decides how many mixnet clients a set of roles takes, so a provider answering every role with the same conduit serves them all over one client while a provider answering with distinct conduits serves each over its own, and the wallet cannot tell those apart. A conduit is anchored to one proven exit for the epoch rather than drawing a fresh path per request, which is why a role's proof survives the client that earned it — but the anchoring is the provider's business and the wallet never learns which exit, or that there is an exit at all. *Avoid*: SlotTunnel and the term "tunnel" generally (the retired name, retired 2026-08-18); ExitAnchored (rejected: it names the wallet's one exit-free type after an exit). Ratified 2026-08-18; the label's capabilities ruled 2026-08-20; see `docs/adr/zingolib/0046-the-wallet-asks-for-a-mixnet-conduit-by-role.md`.

**IndexerSweep exit** — The first exit to prove itself at boot, whose client runs the Server-Selection Sweep. A sweep that reaches no live exit aborts boot rather than opening an indexerless prompt, so "the session opened" and "four exits are proven" are one statement. Ratified 2026-08-17.

**PriceFetch exit** — The second exit to prove itself at boot, whose client fetches the price during boot. The quote is printed and stored nowhere: zingo-cli narrates it and zingo-mobile handles its own result. A later `current_price` runs a fresh fetch over the network and binds this same exit, so it skips the Sentinel rather than reading anything cached. Ratified 2026-08-17.

**IndexerClient exit** — The third exit to prove itself at boot, which carries Transmissions. Its client is the only one that persists past boot; the other three stop when their task ends, leaving their exits proven and role-bound. *Avoid*: Standing Client (the retired name for the persisting client, retired 2026-08-17 when the role moved onto the exit). Ratified 2026-08-17.

**spare exit** — The fourth exit to prove itself at boot, held proven and unassigned so a failing role costs no birth. It is taken up when a proven exit accumulates the ratified threshold of tunnel-phase failures without an intervening success — the same charge rule Health applies to Destinations, where a tunnel-phase failure is the Exit Node's and never charges the Destination. One success redeems, so a transient outage never costs an exit its role. A wave no target answers is the cohort's failure and never the exit's; the Sentinel keeps the harder rule of one silence condemning, because only the exit can fail to answer it. Ratified 2026-08-17.

**Acquisition Race** — The winner-take-all redundancy pattern of an acquisition: the clutch's reservations are the arms, a connect attempt is one pull of an arm, the hedged launch policy schedules the pulls, the first success binds, and every loser is cancelled. Today each arm is pulled at most once. The vocabulary is the source literature's, exactly — arm and pull from the multi-armed bandit (Robbins 1952), racing from Happy Eyeballs (RFC 6555, RFC 8305), hedging from the tail-at-scale hedged request (Dean & Barroso 2013) — and hedging names a launch policy within a race, never the race itself. Code spells the term `acq_race` (`AcqRace` in type names), so identifiers name the Acquisition Race explicitly. Ratified 2026-08-07. See `docs/adr/zingolib/0035-the-acquisition-race-speaks-the-literatures-vocabulary.md`. _Avoid_: bare "race" for an acquisition's race in code identifiers.
...
**Sync Indexer Selection** — The draw that decides which indexer serves a Sync Session: fresh per session from the curated sync list, and never the same operator as any Transmission Destination, in both directions (Destination draws exclude the sync operator per `docs/adr/zingolib/0022`, and the sync draw excludes the session's Destinations). Applies to shielded compact-block sync only: transparent address queries reveal the address set to their indexer, so they keep a sticky route, decided separately. Ratified 2026-07-28.

**Destination Indexer** — An Indexer used only as a Transmission target, drawn at random from a curated Destination list kept separate from the sync-server list. The list holds one endpoint per operator, since the accumulating party Destination Selection defends against is the operator, not the DNS name. Distinct from the sync Indexer that serves compact blocks, and the distinction is an enforced invariant rather than a tendency: every transmission draw excludes the sync indexer's operator from the pool, so the address-knowing sync indexer never receives a Transmission, and a draw with no eligible Destination refuses rather than falling back. Renamed from Broadcast Indexer 2026-08-07. _Avoid_: Broadcast Indexer. See `docs/adr/zingolib/0022-broadcast-witness-never-the-sync-indexer.md`.
...
**zingo-adrs** — The `zingolabs/zingo-adrs` repository: the one append-only home of architecture decision records for every zingolabs code repository. A code repository checks in a submodule pointer to it and none of its content. *Avoid*: "the ledger", "the ADR repo".

**Org-scoped record** — A record that binds every zingolabs code repository. It lives at the top level of zingo-adrs. *Avoid*: global ADR, shared ADR.

**Repo-scoped record** — A record that binds exactly one code repository. It lives in that repository's subdirectory of zingo-adrs (`zingolib/` for zingolib). *Avoid*: local ADR, project ADR.
