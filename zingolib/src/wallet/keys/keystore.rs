...
impl Keystore {
    ...
    // NOTE: (Pacu) determine whether this is needed or not. The equivalent
    // code does not seem to be in the new Zingo codebase.
    ...
    pub fn get_taddr_to_secretkey_map(
        ...
        match self {
            Keystore::InMemory(wc) => wc.get_taddr_to_secretkey_map(&config.chain),
            #[cfg(feature = "ledger-support")]
    ...
    pub(crate) fn get_all_taddrs(&self, config: &ZingoConfig) -> HashSet<String> {
        match self {
            Keystore::InMemory(wc) => wc.get_all_taddrs(&config.chain),
            #[cfg(feature = "ledger-support")]
...
impl Keystore {
    ...
    pub fn transparent_child_addresses(&self) -> Arc<append_only_vec::AppendOnlyVec<(usize, TransparentAddress)>> {
        match self {
            ...
            #[cfg(feature = "ledger-support")]
            Self::Ledger(wc) => wc.transparent_child_addresses.clone(),
        }
    }
}
...
impl TryFrom<&Keystore> for sapling_crypto::note_encryption::PreparedIncomingViewingKey {
    ...
    fn try_from(value: &Keystore) -> Result<Self, Self::Error> {
        match value {
            Keystore::InMemory(_) => sapling_crypto::SaplingIvk::try_from(value)
...
impl TryFrom<&Keystore> for orchard::keys::PreparedIncomingViewingKey {
    ...
    fn try_from(wc: &Keystore) -> Result<Self, String> {
        match wc {
            Keystore::InMemory(_) => orchard::keys::IncomingViewingKey::try_from(wc)
