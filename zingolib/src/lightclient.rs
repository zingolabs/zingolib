...
use zcash_primitives::{
    ...
    memo::{Memo, MemoBytes},
    transaction::{components::amount::NonNegativeAmount, fees::zip317::MINIMUM_FEE, Transaction},
};
...
use crate::{
    ...
    wallet::{
        data::{finsight, summaries::ValueTransferKind},
...
pub mod sync {
    ...
    impl LightClient {
        ...
        async fn start_sync(&self) -> Result<SyncResult, String> {
            ...
            {
                let trees =
                    crate::grpc_connector::get_trees(self.get_server_uri(), last_synced_height)
                        .await?;
            ...
            let latest_blockid = crate::wallet::data::BlockData::new_with(
