...
/// witness-is-never-the-sync-indexer invariant (ADR 0015).
...
/// there is nothing safe to draw, so the send refuses (ADR 0015).
...
/// raw transaction to a drawn indexer (ADR 0015). The sync indexer already
