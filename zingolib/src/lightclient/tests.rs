...
async fn sapling_incoming_sapling_outgoing(scenario: NBlockFCBLScenario) {
    ...
    let extfvk1 =
        ExtendedFullViewingKey::try_from(&*lightclient.wallet.wallet_capability().read().await)
            .unwrap();
    let value = 100_000;
...
async fn multiple_incoming_same_transaction(scenario: NBlockFCBLScenario) {
    ...
    let extfvk1: ExtendedFullViewingKey = (&*lightclient.wallet.wallet_capability().read().await)
        .try_into()
        .unwrap();
    let value = 100_000;
...
async fn sapling_incoming_multisapling_outgoing(scenario: NBlockFCBLScenario) {
    ...
    // 2. Send an incoming transaction to fill the wallet
    let extfvk1 = (&*lightclient.wallet.wallet_capability().read().await)
        .try_into()
        .unwrap();
    let value = 100_000;
