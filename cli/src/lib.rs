...
fn regtest_config_check(regtest_manager: &Option<regtest::RegtestManager>, chain: &BlockChain) {
    if regtest_manager.is_some() && chain == &BlockChain::Regtest {
        println!("regtest detected and network set correctly!");
    } else if regtest_manager.is_some() && chain != &BlockChain::Regtest {
        panic!("Regtest flag detected, but unexpected network set! Exiting.");
    } else if chain == &BlockChain::Regtest {
...
pub fn attempt_recover_seed(_password: Option<String>) {
    ...
    ZingoConfig {
        ...
        chain: zingoconfig::BlockChain::Mainnet,
