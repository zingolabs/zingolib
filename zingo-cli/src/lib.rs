...
use zingo_perspective::config::{ChainType, ClientConfig, DEFAULT_WALLET_NAME, WalletConfig};
use zingo_perspective::data::PollReport;
use zingo_perspective::lightclient::{DEFAULT_REQUEST_TIMEOUT, LightClient};
use zingo_perspective::netutils::Indexer as _;
use zingo_perspective::wallet::WalletSettings;
...
For a NEW wallet created in Offline mode it is instead an optional override of the library's built-in birthday floor."))
            .arg(Arg::new("server")
                ...
                .default_value(zingo_perspective::config::DEFAULT_INDEXER_URI))
            .arg(Arg::new("offline")
                .long("offline")
                .action(clap::ArgAction::SetTrue)
...
fn decide_connectivity(
    ...
    stored: zingo_perspective::connectivity::ConnectivityConsent,
    ...
    match stored {
        zingo_perspective::connectivity::ConnectivityConsent::StandingOnline => {
            ConnectivityDecision::Online { store: false }
        }
        zingo_perspective::connectivity::ConnectivityConsent::Unrecorded => {
...
fn get_communication_mode(matches: &clap::ArgMatches) -> std::io::Result<CommunicationMode> {
    ...
    if matches.get_flag("forget-online") {
        zingo_perspective::connectivity::forget_connectivity_consent(&data_dir)?;
...
fn build_zingo_config(filled_template: &ConfigTemplate) -> std::io::Result<ClientConfig> {
    ...
    let wallet_config = if let Some(seed_phrase) = filled_template.seed.clone() {
        ...
        let chain_height = match filled_template.server.clone() {
            Some(server) => RT
                .block_on(async move {
                    zingo_perspective::netutils::GrpcIndexer::new(server)
                        .await
                        .map_err(|e| format!("{e:?}"))?
            ...
            None => match u32::try_from(filled_template.birthday) {
                ...
                _ => zingo_perspective::config::lib_birthday(filled_template.chaintype),
            },
        };
...
pub(crate) fn startup(filled_template: &ConfigTemplate) -> std::io::Result<CommandChannel> {
    ...
    if filled_template.communication_mode == CommunicationMode::Online {
        use zingo_perspective::nym::{MixnetStartPolicy, ProvisionStrategy};
        ...
        RT.spawn(async move {
            ...
            while status_rx.changed().await.is_ok() {
                ...
                match status.mode {
                    zingo_perspective::nym::MixnetMode::Ready => info!(
                        "Mixnet Mode ready; send and price-fetch route over the mixnet \
                         (see `nym status`)."
                    ),
                    zingo_perspective::nym::MixnetMode::Died => {
                        let cause = status
                            .death
                            .as_ref()
...
fn print_salvaged_recovery_info(
    ...
    let recovery_info = zingo_perspective::wallet::LightWallet::read_recovery_info(
