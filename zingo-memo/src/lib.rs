...
mod tests {
    ...
    fn parse_zingo_memo_version_n() {
        for test_vector in zingomemo_vectors::UA_TEST_VECTORS {
            ...
            // version0
            #[allow(deprecated)]
            let version0_bytes = create_wallet_internal_memo_version_0(std::slice::from_ref(&ua)).unwrap();
            ...
            let version1_bytes =
                create_wallet_internal_memo_version_1(std::slice::from_ref(&ua), &random_rejection_indexes)
