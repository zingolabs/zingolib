...
Status: accepted. The decision is scoped to Phase 1 (note splitting) and
supersedes the note-splitting role of the `continue_note_splitting` /
`MigrationState` driver for consumers. Phase 2 (parts, buckets, their consent
...
`AwaitingConfirmation`, or `Complete`. The consumer loops (sync, `quick_split`,
repeat) until `Complete`, previews with `plan_note_split(account)`, and
observes per-transaction progress through `split_progress_handle()`. Note
...
notes, `quick_split` executes its first round, and a note already sized
...
after a partial broadcast. That makes a persisted driver unnecessary. The
prior surface (`start_ironwood_migration` → `continue_note_splitting`
returning `SplitStep`, backed by
...
consumer's loop count) rather than state worth persisting.
...
- **The inter-round confirmation barrier moves to the consumer.** A round's
  ...
  round **before it plans** and returns `AwaitingConfirmation` if one is
  found. This is load-bearing: while a round is unconfirmed its inputs have
  left the spendable set and its outputs are not yet spendable, so a replan
  sees nothing left to split and would report a false `Complete`. The
  ...
  *outputs*, which the wallet records at build time, before transmit. That
  replaces the stored `pending_txids`.
- **No consent-hash binding for Phase 1.** Splits reveal no value (both ends
  ...
  state, and this decision does not touch them.

## Considered and rejected

- **An explicit propose→send pair** (`propose_note_split` → `send_note_split`,
  mirroring `propose_send` / `send_stored_proposal`, ADR 0006). It guarantees
  the executed round is exactly the previewed one by threading a proposal
  value. We rejected it: once the round counter dissolved, a back-to-back
  ...
  matches `quick_immediate_migration` verbatim. A `Proposal` also cannot span
  rounds: its multi-step form chains *transparent* outputs by outpoint, while
  a split's shielded outputs have no witness until mined, so the explicit form
  would still be one proposal per round.
- **Keeping `continue_note_splitting` / `MigrationState` for consumers.** We
  rejected this for Phase 1: it persists a state machine that the memoryless
  replan makes redundant and that the mobile FFI would have to mirror and
  resume.
