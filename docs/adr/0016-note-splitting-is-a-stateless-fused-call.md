...
for executing Phase 1 note splitting. Like the immediate Drain (ADR 0018) and
...
repeat) until `Complete`. Preview with `plan_note_split(account)`, and observe
per-transaction progress through `split_progress_handle()`. Note
...
- **The inter-round confirmation barrier moves to the consumer.** A round's
  ...
  This is load-bearing: while a round is unconfirmed its inputs have left the
  spendable set and its outputs are not yet spendable, so a replan sees nothing
  left to split and would report a false `Complete`. The in-flight signal is
  **derived** from the wallet's pending transactions, specifically the round's
  own unconfirmed, account-owned V2 Orchard *outputs*, which the wallet records
  at build time, before transmit. That replaces the stored `pending_txids`.
...
- **An explicit propose→send pair** (`propose_note_split` → `send_note_split`,
  ...
  rounds, because its multi-step form chains *transparent* outputs by outpoint
  while a split's shielded outputs have no witness until mined, so the explicit
  form would still be one proposal per round.
