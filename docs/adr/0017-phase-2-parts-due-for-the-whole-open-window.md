# Phase 2 parts are due for the whole open window; the random target is advisory

Status: accepted. A deliberate ZIP 318 privacy relaxation the product owner
accepted for responsiveness, not a bug fix. Scoped to Phase 2 (scheduled
parts).

A scheduled part is due to broadcast for its **entire open window** — it is in
the current bucket, whose boundary sits at or below the tip — rather than only
once the synced tip reaches its random `target_height`. The target is still
computed, stored, and exposed through `BroadcastWindow::latest_target_unix_time`,
read-only, as the user-reminder hint; it no longer gates sendability. And
...
- `first_permitted_bucket` (the `+ 1` chooser) still governs **expiry rebuild
  and reassignment**, which target a fresh *future* window, not the current
  one. Only initial scheduling — and `reschedule_parts`, which re-runs it
