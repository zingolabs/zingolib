...
wallet its funds — the seed is recoverable from the prefix (the salvage floor,
below) — but it costs local transaction metadata and a full rescan.
...
- Version 42 is disambiguated by an end-of-file-anchored dual parse of the
  ...
  section, the only region where the two layouts differ: the pre-release layout
  opens it with one extra settings byte. The pre-release reading is consulted
  only when the canonical reading fails to parse, so a file written by current
  code never loads through the fallback.
...
version gate — or, where a number is already double-assigned, the disambiguation
— and the regression tests that pin every shipped revision to a loading wallet.
