...
//!
//! Every covered network operation — the price fetch, the broadcast fan-out,
//! the attach validation, and the sync-path connectivity probe — reports its
//! failures as data: which stage failed, against what target, with the full
//! cause chain carried as one text per layer. One taxonomy serves them all,
//! so a consumer that learns to read one operation's failures can read every
//! operation's.
//!
//! The crate is std-only with zero dependencies, a hard requirement: it is
//! what lets one crate serve two lockfile-isolated cargo workspaces (the
//! parent workspace and the standalone `zingo-netutils` workspace) without
//! resolver coupling. Classification into these types is done by pure
//! functions owned by the crates that own the error types being classified
//! (`zingo-price` classifies `reqwest::Error`, zingolib classifies
//! `Socks5TransmitError`); this crate holds only the taxonomy and the
//! generic chain inspector.
