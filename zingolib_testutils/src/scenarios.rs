...
/// the `zcash_local_net` boundary. The infrastructure has no Ironwood miner
/// pool yet.
fn miner_pool(mine_to_pool: PoolType) -> MinerPool {
    match mine_to_pool {
        ...
        PoolType::Shielded(ShieldedPool::Orchard) => MinerPool::Orchard,
        PoolType::Shielded(ShieldedPool::Ironwood) => {
            unimplemented!("zcash_local_net has no ironwood miner pool")
        }
    }
}
...
/// that zingolib configuration consumes. Inverse of [`net_activation_heights`],
/// for values coming back across the `zcash_local_net` boundary (e.g.
/// `Validator::get_activation_heights`).
